...
const HERMES_SHIM_TEMPLATE: &str = r#"#!/usr/bin/env bash
...
command -v {BIN} >/dev/null 2>&1 || { echo '{}'; exit 0; }
exec {BIN} hook hermes pre_tool_call --scope all
"#;

fn hermes_shim_body(binary: &str) -> String {
    HERMES_SHIM_TEMPLATE.replace("{BIN}", binary)
}

pub fn run(root: &Path, rules: &[Rule]) -> Result<Vec<String>, String> {
    let mut written = Vec::new();
    let binary = config::hook_binary(&config::repo_config_path(root));
    ...
    write_cursor(&root.join(".cursor/hooks.json"), "repo", &binary, &mut written)?;
    write_hermes_shim(root, &binary, &mut written)?;
    write_git_hook(root, &binary, &mut written)?;
...
/// Generated hooks are committed; the binary they call is not. A teammate who
/// has not installed stele would otherwise get a hook failure on every event in
/// every harness — which is how a team decides to delete the hooks. So each
/// generated command first tests for the binary and exits 0 (allow, silent)
/// when it is absent. CI deliberately does NOT get this guard: there, a missing
/// stele must fail loud.
fn hook_cmd(binary: &str, harness: &str, event: &str, scope: &str) -> String {
    format!(
        "command -v {binary} >/dev/null 2>&1 || exit 0; \
         {binary} hook {harness} {event} --scope {scope}"
    )
}

/// Identifies a stele-owned hook entry independently of which binary path it
/// calls and whether it carries the not-installed guard. Matching on the
/// argument tail keeps `compile` idempotent and `uninstall` complete across
/// every generation of hook we have written: the original scope-less
/// `stele hook claude-code stop`, the `--scope`-bearing form, and a guarded
/// command naming a project-local binary. Deliberately stops before `--scope`
/// so the oldest form still migrates instead of being duplicated.
fn owned_marker(harness: &str, event: &str) -> String {
    format!(" hook {harness} {event}")
}

/// The same ownership test, for every event of one harness.
fn owned_by_harness(harness: &str) -> String {
    format!(" hook {harness} ")
}
...
fn merge_claude_style(
...

    {
        ...
        for (event, cmd_event) in [
            ...
            let cmd = hook_cmd(binary, harness, cmd_event, scope);
            let entry = json!({
                "matcher": "",
            ...
            let owned = owned_marker(harness, cmd_event);
...
fn write_cursor(
    ...
    for (event, cmd_event) in [
        ...
        let cmd = hook_cmd(binary, "cursor", cmd_event, scope);
        let owned = owned_marker("cursor", cmd_event);
...
/// Hermes is detected.
fn write_hermes_shim(root: &Path, binary: &str, written: &mut Vec<String>) -> Result<(), String> {
    let shim = root.join(".stele/hermes-shim.sh");
    write_file(&shim, &hermes_shim_body(binary), written)?;
    make_executable(&shim);
    Ok(())
}

fn write_git_hook(root: &Path, binary: &str, written: &mut Vec<String>) -> Result<(), String> {
    ...
    let body = format!(
        "#!/usr/bin/env bash\n{HOOK_MARKER}\n\
         ...
         #\n\
         # A teammate without stele installed pushes normally — CI still holds\n\
         # the line, so a missing binary delays enforcement, never removes it.\n\
         command -v {binary} >/dev/null 2>&1 || exit 0\n\
         exec {binary} check --quiet-green --scope repo\n"
    );
...
fn install_global_with_state(home: &Path, state_dir: &Path) -> Result<Vec<String>, String> {
    ...
    // Personal hooks fire in every repository, so their binary comes from the
    // personal config — never from whichever repo happens to be open.
    let binary = config::user_config_path()
        .map(|path| config::hook_binary(&path))
        .unwrap_or_else(|_| config::DEFAULT_BINARY.to_string());
    ...
    if let Some(update) = hermes {
        apply_hermes_install(update, &binary, &mut written)?;
    }
    Ok(written)
}
...
pub fn install_hermes_at(home: &Path, state_dir: &Path) -> Result<(), String> {
    ...
    let binary = config::user_config_path()
        .map(|path| config::hook_binary(&path))
        .unwrap_or_else(|_| config::DEFAULT_BINARY.to_string());
    apply_hermes_install(update, &binary, &mut ignored)
}
...
fn apply_hermes_install(
    update: HermesInstall,
    binary: &str,
    written: &mut Vec<String>,
) -> Result<(), String> {
    write_file(&update.shim, &hermes_shim_body(binary), written)?;
    ...
    let hooks_empty;
    {
        ...
        for event in events {
            ...
            for group in arr.iter_mut() {
                ...
                let owned = owned_by_harness(harness);
                handlers.retain(|handler| {
                    !handler["command"]
                        .as_str()
                        .is_some_and(|command| command.contains(&owned))
                });
...
fn remove_cursor(path: &Path, written: &mut Vec<String>) -> Result<(), String> {
    ...
    if let Some(hooks) = doc.get_mut("hooks") {
        ...
        for event in events {
            ...
            let before = arr.len();
            let owned = owned_by_harness("cursor");
            arr.retain(|entry| {
                !entry["command"]
                    .as_str()
                    .is_some_and(|command| command.contains(&owned))
